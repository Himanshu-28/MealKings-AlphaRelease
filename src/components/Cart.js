    ...
    useEffect(() => {
        const fetchCartData = async () => {
            try {
                ...
                const response = await axios.get('http://localhost:8888/cart/1/items'); // Example endpoint
            ...
            <div className="list-group">
                {Object.keys(cart).map((key) => (
                    <div className="list-group-item" key={key}>
                        ...
                        <button className="btn btn-secondary" onClick={() => dispatch(removeItem(cart[key]))}>Remove</button>
                    </div>
                ))}
            </div>
