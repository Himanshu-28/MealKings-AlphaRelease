...
const OrderPlacePage = () => {
  const [cartItems, setCartItems] = useState([]); // Initialize as an array for cart items
  ...
  useEffect(() => {
    ...
    // Fetch user data
    axios.get("http://localhost:8083/customer/1") // Update this URL to fetch user data
      .then((response) => {
        const user = response.data; // The response should contain user data directly
        setUserData({
          username: user.name, // Adjust based on the response structure
          address: user.address,
          address2: "", // Assuming no second address is provided
          city: user.city || "", // Adjust if city is provided in the user data
          state: user.state || "", // Adjust if state is provided in the user data
          zip: "", // Assuming zip is not provided in the user data
          mobile: user.mobileNo // Assuming mobile number is available
        });
      })
  ...
  const continueToPayment = () => {
    if (isAddressSaved) {
      navigate('/payment', { state: { cartItems, finalPrice,userAddress,totalPrice,discountCode: couponCode  } });
    } else {
    ...
    <div className="container-fluid bg-light" style={{ height: "100vh" }}>
      <div className="row justify-content-center align-items-center">
        <div className="col-md-8 bg-white p-5 rounded shadow">
            ...
            <ul className="list-group list-group-flush">
              {cartItems.map((item) => (
                <li key={item.id} className="list-group-item d-flex justify-content-between align-items-center">
                  <span>Item ID: {item.id} - Quantity: {item.quantity} </span>
              ...
              <ul className="list-group">
                {discounts.map((discount) => (
                  <li key={discount.discountCode} className="list-group-item">
                    <span>{discount.discountCode}: {discount.discountPercentage}% off (Valid until {discount.endDate})</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          {/* Final Price and Continue to Payment Button */}
          <div className="card mb-4">
            <h2 className="card-header text-primary">Final Price</h2>
            <h5 className="card-body">Total After Discount: <strong>Rs.{finalPrice}</strong></h5>
